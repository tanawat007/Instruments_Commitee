...
const ORIGINAL_URL = 'https://script.google.com/macros/s/AKfycbwRFRomwIx8BJxdje6SmW3ZmlbaxKJR_wBTNjIql7OBnBywTnSGd9v5GXeUuY9vAyBKEQ/exec';
...
const processconnectionInput = document.getElementById('processconnection');
  ...
  filteredData = allData.filter(row => {
    ...
    if (query) {
      const searchFields = [
        ...
        row.subtype,
        row.scalerange,
        row.processconnection,
        row.equipmentclass,
        row.layerofprotection,
        row.specification
...
function renderTable() {
  ...
  pageData.forEach((row) => {
    ...
    tr.innerHTML = `
      ...
      <td class="font-semibold text-sm">${escapeHtml(row.subtype || '-')}</td>
      <td class="font-semibold text-sm">${escapeHtml(row.scalerange || '-')}</td>
      <td class="font-semibold text-sm">${escapeHtml(row.processconnection || '-')}</td>
      ...
      <td class="text-right">
        <div class="flex justify-end gap-1">
          <button class="view-btn btn btn-xs btn-ghost text-info tooltip tooltip-left" data-tip="ดูรายละเอียด"
                  ...
                  data-subtype="${escapeAttr(row.subtype || '')}"
                  data-scalerange="${escapeAttr(row.scalerange || '')}"
                  data-processconnection="${escapeAttr(row.processconnection || '')}"
                  ...
                  data-specification="${escapeAttr(row.specification || '')}">
  ...
  document.querySelectorAll('.edit-btn').forEach((btn) => {
    btn.addEventListener('click', function () {
      ...
      editRecord(
        ...
        this.dataset.processconnection,
...
if (form) {
  form.addEventListener('submit', async (e) => {
    ...
    const params = {
      ...
      processconnection: processconnectionInput?.value.trim() || '',
    ...
    try {
      ...
      if (result && result.success === true) {
        ...
        window.location.href = '03_localgauge.html';
...
function editRecord(id, materialCode, description, keepingArea, objecttype, classtype, subtype, scalerange, processconnection, equipmentclass, layerofprotection, dateOfUse, expiredDate, specification) {
  ...
  if (!form) {
    sessionStorage.setItem('editRecord', JSON.stringify({
      id, materialCode, description, keepingArea, objecttype, classtype, subtype, scalerange, processconnection, equipmentclass, layerofprotection, dateOfUse, expiredDate, specification
    }));
    window.location.href = 'createform_localgauge.html?edit=1';
    return;
  }
  ...
  if (processconnectionInput) processconnectionInput.value = processconnection || '';
  ...
  if (specificationInput) specificationInput.value = specification || '';
...
function viewRecord(data) {
  ...
  setEl('detail-subtype', data.subtype || '-');
  setEl('detail-scalerange', data.scalerange || '-');
  setEl('detail-processconnection', data.processconnection || '-');
  ...
  setEl('detail-specification', data.specification || '-');
