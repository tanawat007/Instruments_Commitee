...
const ORIGINAL_URL = 'https://script.google.com/macros/s/AKfycbwQPkFkAxkTOyUbHu-GHyAgwdZ9V15nXCdpXiUQ5EDOItm2rm5ZBNJbLEC-U8f-SgKP/exec';
...
function renderTable() {
  ...
  pageData.forEach((row) => {
    ...
    const quantityVal = (row.Quantity || '').trim();
    let quantityLink;
    if (quantityVal && quantityVal.toLowerCase() !== 'na') {
      quantityLink = `<a href="${escapeHtml(quantityVal)}" target="_blank" class="btn btn-xs btn-outline btn-warning gap-1">
           <i class="fa-solid fa-file-shield text-[10px]"></i> Instrument List
         </a>`;
    } else if (quantityVal.toLowerCase() === 'na') {
      quantityLink = '<span class="badge badge-ghost badge-sm">NA</span>';
    } else {
      quantityLink = '<span class="text-slate-300 text-xs">-</span>';
    }
    
    const lifttimeVal = (row.Lifttime || '').trim();
    let lifttimeLink;
    if (lifttimeVal && lifttimeVal.toLowerCase() !== 'na') {
      lifttimeLink = `<a href="${escapeHtml(lifttimeVal)}" target="_blank" class="btn btn-xs btn-outline btn-warning gap-1">
           <i class="fa-solid fa-file-shield text-[10px]"></i> Circuit Diagram
         </a>`;
    } else if (lifttimeVal.toLowerCase() === 'na') {
      lifttimeLink = '<span class="badge badge-ghost badge-sm">NA</span>';
    } else {
      lifttimeLink = '<span class="text-slate-300 text-xs">-</span>';
    }

    const leakageClassVal = (row.LeakageClass || '').trim();
    let leakageClassLink;
    if (leakageClassVal && leakageClassVal.toLowerCase() !== 'na') {
      leakageClassLink = `<a href="${escapeHtml(leakageClassVal)}" target="_blank" class="btn btn-xs btn-outline btn-warning gap-1">
           <i class="fa-solid fa-file-shield text-[10px]"></i> Logic Diagram
         </a>`;
    } else if (leakageClassVal.toLowerCase() === 'na') {
      leakageClassLink = '<span class="badge badge-ghost badge-sm">NA</span>';
    } else {
      leakageClassLink = '<span class="text-slate-300 text-xs">-</span>';
    }
    ...
    if (msdsVal && msdsVal.toLowerCase() !== 'na') {
      msdsLink = `<a href="${escapeHtml(msdsVal)}" target="_blank" class="btn btn-xs btn-outline btn-warning gap-1">
           <i class="fa-solid fa-file-shield text-[10px]"></i> Specification
         </a>`;
