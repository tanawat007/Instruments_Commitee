...
function updateStats() {
  ...
  // Populate partname filter
  const filterPartname = document.getElementById('filter-partname');
  if (filterPartname) {
    const current = filterPartname.value;
    const partnamesSet = new Set();
    allData.forEach(row => {
      const partname = (row.Description || '').trim();
      if (partname) partnamesSet.add(partname);
    });
    filterPartname.innerHTML = '<option value="">ทุก Part Name</option>';
    [...partnamesSet].sort().forEach(partname => {
      const opt = document.createElement('option');
      opt.value = partname;
      opt.textContent = partname;
      filterPartname.appendChild(opt);
    });
    filterPartname.value = current;
  }
  ...
  if (filterType) {
    ...
    allData.forEach(row => {
      const type = (row.Quantity || '').trim();
      if (type) typesSet.add(type);
    });
    filterType.innerHTML = '<option value="">ทุก Type</option>';
    [...typesSet].sort().forEach(type => {
      const opt = document.createElement('option');
      opt.value = type;
  ...
  // Populate dialsize filter
  const filterDialsize = document.getElementById('filter-dialsize');
  if (filterDialsize) {
    const current = filterDialsize.value;
    const dialsizesSet = new Set();
    allData.forEach(row => {
      const dialsize = (row.Lifttime || '').trim();
      if (dialsize) dialsizesSet.add(dialsize);
    });
    filterDialsize.innerHTML = '<option value="">ทุก Dial Size</option>';
    [...dialsizesSet].sort().forEach(dialsize => {
      const opt = document.createElement('option');
      opt.value = dialsize;
      opt.textContent = dialsize;
      filterDialsize.appendChild(opt);
    });
    filterDialsize.value = current;
  }

  // Populate range filter
  const filterRange = document.getElementById('filter-range');
  if (filterRange) {
    const current = filterRange.value;
    const rangesSet = new Set();
    allData.forEach(row => {
      const range = (row.LeakageClass || '').trim();
      if (range) rangesSet.add(range);
    });
    filterRange.innerHTML = '<option value="">ทุก Range</option>';
    [...rangesSet].sort().forEach(range => {
      const opt = document.createElement('option');
      opt.value = range;
      opt.textContent = range;
      filterRange.appendChild(opt);
    });
    filterRange.value = current;
  }

  // Populate processconnection filter
  ...
  if (filterProcessconnection) {
    ...
    const processconnectionsSet = new Set();
    allData.forEach(row => {
      const processconnection = (row.FailAction || '').trim();
      if (processconnection) processconnectionsSet.add(processconnection);
    });
    filterProcessconnection.innerHTML = '<option value="">ทุก Process Connection</option>';
    [...processconnectionsSet].sort().forEach(processconnection => {
      const opt = document.createElement('option');
...
function applyFilters() {
  ...
  const partnameFilter = document.getElementById('filter-partname')?.value || '';
  const typeFilter = document.getElementById('filter-type')?.value || '';
  const dialsizeFilter = document.getElementById('filter-dialsize')?.value || '';
  ...
  filteredData = allData.filter(row => {
    ...
    // Partname filter
    if (partnameFilter && (row.Description || '').trim() !== partnameFilter) return false;
    // Type filter
    if (typeFilter && (row.Quantity || '').trim() !== typeFilter) return false;
    // Dialsize filter
    if (dialsizeFilter && (row.Lifttime || '').trim() !== dialsizeFilter) return false;
    // Range filter
    if (rangeFilter && (row.LeakageClass || '').trim() !== rangeFilter) return false;
    // Processconnection filter
    if (processconnectionFilter && (row.FailAction || '').trim() !== processconnectionFilter) return false;
    return true;
  });
